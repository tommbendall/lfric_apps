...
!!        (lhs_uv, lhs_w) = norm_u*(Mu*u - P2t*t - grad*p),
!!        lhs_p = M3p*p - P3t*t + Q32*u,
!!        with t = -Mt^(-1) * Pt2*u
module apply_mixed_operator_kernel_mod
...
type, public, extends(kernel_type) :: apply_mixed_operator_kernel_type
  ...
  contains
  procedure, nopass :: apply_mixed_operator_code
end type

!-------------------------------------------------------------------------------
! Contained functions/subroutines
!-------------------------------------------------------------------------------
public :: apply_mixed_operator_code
...
subroutine apply_mixed_operator_code(cell,                       &
                                     nlayers,                    &
                                     lhs_uv, lhs_w, lhs_p,       &
  ...
  ! Fields
  real(kind=r_solver), dimension(undf_w2h), intent(inout) :: lhs_uv
  ...
  do df2 = 1, ndf_w2
    do df = 1, ndf_w2h
      ...
      lhs_uv(iw2h:iw2h+nm1) = lhs_uv(iw2h:iw2h+nm1) &
                            + norm_u(iw2:iw2+nm1)*  &
                              mu_cd(ij:ij+nm1, df, df2)*u_e(:,df2)
    end do
  end do
...
end module apply_mixed_operator_kernel_mod
