...
module apply_variable_hx_kernel_mod
  ...
  interface apply_variable_hx_code
    module procedure &
      apply_variable_hx_code_r_single, &
      apply_variable_hx_code_r_double
  end interface
...
!> @param[in] const_div Scaling constant (tau_r*dt) applied to the divergence term
!> @param[in] const_theta Scaling constant (tau_t*dt) applied to the potential
!!            temperature term
...
! R_DOUBLE PRECISION
! ==================
subroutine apply_variable_hx_code_r_double(cell,                    &
                                           nlayers,                 &
                                           lhs, x,                  &
                                           mt_inv,                  &
                                           pressure,                &
  ...
  real(kind=r_double), dimension(ndf_w3) :: lhs_e, p_e
  ...
  do k = 0,nlayers-1
    ...
    lhs_e = matmul(m3(ik,:,:),p_e) + const_div*matmul(div(ik,:,:),x_e) &
                                   + const_theta*matmul(p3t(ik,:,:),t_e)
    do df = 1,ndf_w3
  ...
  deallocate( t )
end subroutine apply_variable_hx_code_r_double

! R_SINGLE PRECISION
! ==================
subroutine apply_variable_hx_code_r_single(cell,                    &
                                           nlayers,                 &
                                           lhs, x,                  &
                                           mt_inv,                  &
                                           pressure,                &
  ...
  real(kind=r_single), dimension(ndf_w3) :: lhs_e, p_e
  ...
  deallocate( t )
end subroutine apply_variable_hx_code_r_single
